...
const PreviousCollections = () => {
  ...
  useEffect(() => {
    const fetchPreviousCollections = async () => {
      try {
        const response = await axios.get('https://nourishnet-1hvx.onrender.com/donate/previousCollectionsForVolunteer');
        setPreviousCollectionsData(response.data);
      } catch (error) {
        console.error('Error fetching previous collections:', error);
      }
  ...
  const handleDistribute = async (donationId) => {
    try {
      await axios.put(`https://nourishnet-1hvx.onrender.com/distributeDonation/${donationId}`);
