...
const PendingCollections = () => {
  ...
  useEffect(() => {
    const fetchPendingCollections = async () => {
      try {
        const response = await axios.get('https://nourishnet-1hvx.onrender.com/donate/pendingCollectionsForVolunteer');
  ...
  const handleCollect = async (donationId) => {
    try {
      await axios.put(`https://nourishnet-1hvx.onrender.com/donate/collectDonation/${donationId}`);
