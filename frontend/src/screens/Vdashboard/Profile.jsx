...
const UserProfile = () => {
  ...
  const fetchUserData = async () => {
    try {
      // Use the user ID stored in localStorage to fetch user data
      const userId = localStorage.getItem('userId');
      const response = await axios.get(`https://nourishnet-1hvx.onrender.com/users/${userId}`);
  ...
  const handleUpdateProfile = async () => {
    try {
      // Use the user ID stored in localStorage to update user data
      const userId = localStorage.getItem('userId');
      await axios.put(`https://nourishnet-1hvx.onrender.com/users/${userId}`, userData);
