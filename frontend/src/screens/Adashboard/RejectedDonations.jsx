...
const RejectedDonations = () => {
  ...
  const fetchRejectedDonations = async () => {
    try {
      const response = await axios.get('https://nourishnet-1hvx.onrender.com/donate/rejecteddonations');
  ...
  const handleCancel = async (donationId) => {
    try {
      const response = await axios.put(`https://nourishnet-1hvx.onrender.com/donate/returnpending/${donationId}`);
