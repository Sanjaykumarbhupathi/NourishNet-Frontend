...
const PendingDonations = () => {
  ...
  useEffect(() => {
    const fetchPendingDonations = async () => {
      try {
        const response = await axios.get('https://nourishnet-1hvx.onrender.com/donate/pendingdonationsforadmin');
  ...
  const handleAccept = async (donationId) => {
    try {
      const response = await axios.put(`https://nourishnet-1hvx.onrender.com/donate/accept/${donationId}`);
  ...
  const handleReject = async (donationId) => {
    try {
      const response = await axios.put(`https://nourishnet-1hvx.onrender.com/donate/reject/${donationId}`);
      const updatedDonation = response.data;
      // Update the state to reflect the new status
      setPendingDonations((prevDonations) =>
        prevDonations.map((donation) =>
