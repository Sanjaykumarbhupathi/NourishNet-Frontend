...
const PendingDonations = () => {
  ...
  const handleCancelClick = async (donationId) => {
    try {
      ...
      const response = await axios.delete(`https://nourishnet-1hvx.onrender.com/donate/${donationId}`);
  ...
  useEffect(() => {
    ...
    const fetchPendingDonations = async () => {
      try {
        const response = await axios.get('https://nourishnet-1hvx.onrender.com/donate/pendingdonationsbyUser');
